...
public class StringUtil {
   ...
   /* public static String getHtmlText(String html) {
    ...
    public static final String upperLower = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz";// O,I去掉
    public static final String lowerDigit = "abcdefghijkmnpqrstuvwxyz23456789";// o,l去掉,0,1去掉
    public static final String upperDigit = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";// O,I去掉,0,1去掉
    ...
    public static String getUpperRandomString(int length) {
        if (length > 100) length = 100;
        return getRandomString(upper, length);
    }

    public static String getLowerRandomString(int length) {
        if (length > 100) length = 100;
        return getRandomString(lower, length);
    }
    ...
    public static String getRandomString(int length, boolean includeUpper, boolean includeLower, boolean includeDigital) {
        ...
        if(!includeUpper && includeLower && includeDigital){
            return getRandomString(lowerDigit, length);
        }
        if(!includeUpper && includeLower && !includeDigital){
            return getRandomString(lower, length);
        }
        ...
        throw new RuntimeException("必须包含一种字符！");
    }
    /**
     * @param length
     *            生成的字符串长度，<100
